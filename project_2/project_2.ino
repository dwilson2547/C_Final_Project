    ...
    while (client.connected()) {
      ...
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          read_csv_file("temp.csv", csv_field, 3);
          client.println("</table>");
          //Output HTML Bottom Content to client
          html_file_output("bottom.htm");
        }
        if (c == '\n') {
          // Starting a new line
...
void html_file_output(const char * filename) {
  File filedata = SD.open(filename);
  delay(1);
  int n=0;
  int i=0;
  char htmlbuffer[1400];
  if(filedata.available()) {
      ...
      while(filedata.available()) {
        i = 0;
        while(filedata.available() && i<1400) { 
          htmlbuffer[i] = filedata.read();
          if(htmlbuffer[i]=='\n') {
            n++;
          }
          i++;
...
void read_csv_file(String filename, int fieldCount, int recordCount) {
  File filedata = SD.open(filename);
  int n;
  int i=0;
  int j;
